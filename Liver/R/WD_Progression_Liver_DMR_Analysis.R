# Liver WD Progression DMR Analysis ####
# Charles Mordaunt
# 3/23/18
...
ggheatmap2_pheno <- function(x, phenoData, hm.colours=my.colours, my.values, low, high) {
        ...
        phenoData.plot <- ggplot(phenoData_m, aes(Sample, variable)) +
                ...
                scale_color_manual(breaks = c("Early WD", "Advanced WD", "Male", "Female"), 
                                   values = c("Early WD"="#3366CC", "Advanced WD"="#FF3366", "Male"="#FFFF33", "Female"="#FF6633")) +
                scale_fill_manual(breaks = c("Early WD", "Advanced WD", "Male", "Female"), 
                                  values = c("Early WD"="#3366CC", "Advanced WD"="#FF3366", "Male"="#FFFF33", "Female"="#FF6633"))
...
ttest <- subset(ttest, abs(meanDiff) >= minDiff & minReads >= coverage & pValue <= 0.05) # 12280 -> 2458
write.table(ttest, "Tables/Liver WDEvWDA DMRs Stats.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
# Subset
backCov <- subset(backCov, minReads >= coverage) #463351 -> 158301
background <- backCov[,c("chr", "start", "end")]
background <- background[order(background$chr, background$start),]
write.table(background, "UCSC Tracks/Liver WDEvWDA Subsetted Background.bed", sep = "\t", quote = FALSE, row.names = FALSE, col.names=FALSE)
...
# Make bed files
DMRs_all <- GRangetoBED(GR=GR_DMRs_all, writeFile=TRUE, fileName="UCSC Tracks/Liver_WDEvWDA_DMRs.bed")
DMRs_hyper <- GRangetoBED(GR=GR_DMRs_hyper, writeFile=TRUE, fileName="UCSC Tracks/Liver_WDEvWDA_Hyper_DMRs.bed")
DMRs_hypo <- GRangetoBED(GR=GR_DMRs_hypo, writeFile=TRUE, fileName="UCSC Tracks/Liver_WDEvWDA_Hypo_DMRs.bed")
...
prepGREAT(DMRs=GR_DMRs_all, Background=GR_Background, fileName="UCSC Tracks/Liver_WDEvWDA_DMRs_hg19_for_GREAT.bed", writeBack=TRUE,
          backName="UCSC Tracks/Liver_WDEvWDA_Background_hg19_for_GREAT.bed")
prepGREAT(DMRs=GR_DMRs_hyper, Background=GR_Background, fileName="UCSC Tracks/Liver_WDEvWDA_Hyper_DMRs_hg19_for_GREAT.bed")
prepGREAT(DMRs=GR_DMRs_hypo, Background=GR_Background, fileName="UCSC Tracks/Liver_WDEvWDA_Hypo_DMRs_hg19_for_GREAT.bed")
...
#colnames(methdiff) <- c(paste("WDE", 1:3,sep=""), paste("WDA", 1:7, sep=""))
...
pdf(file="Figures/WDEvWDA DMRs Heatmap.pdf", width=10, height=8, onefile = FALSE)
...
info$WDStage[info$Sequencing.ID %in% c("VMDK4B","VMDK5B", "VMDK2B")] <- "Early WD"
info$WDStage[info$Sequencing.ID %in% c("VMDK3B", "VMDK2A", "VMDK4A", "VMDK1A", "VMDK1B", "VMDK5A", "VMDK3A")] <- "Advanced WD"
info$WDStage[is.na(info$WDStage)] <- "None"
phenoData <- info[,c("Sequencing.ID", "WDStage", "Sex")]
colnames(phenoData)[1] <- "Sample"
phenoData <- subset(phenoData, WDStage %in% c("Early WD", "Advanced WD"))
...
phenoData$WDStage <- factor(phenoData$WDStage, levels=c("Early WD", "Advanced WD"), ordered=TRUE)
...
pdf(file="Figures/Liver WDEvWDA DMRs Heatmap phenoData.pdf", width=10, height=8, onefile = FALSE)
ggheatmap.show_pheno(methplot)
dev.off()

# PCA Plot ####
data <- t(as.matrix(meth_heat))
diagnosis <- c(rep("Early WD", 3), rep("Advanced WD", 7))
...
g + 
        ...
        theme(legend.direction = 'horizontal', legend.position = c(0.68, 0.95), panel.grid.major = element_blank(), 
        ...
        xlab("PC1 (65% of Variance)") +
        ylab("PC2 (7% of Variance)") +
        scale_color_manual(breaks = c("Early WD", "Advanced WD"), values = c("Early WD"="#3366CC", "Advanced WD"="#FF3366")) +
        scale_x_continuous(breaks=pretty_breaks(n=4)) +
        scale_y_continuous(breaks=pretty_breaks(n=4)) +
        geom_point(aes(color = diagnosis), size=3)
ggsave("Figures/Liver WDEvWDA DMRs PCA plot.png", dpi = 600, width = 8, height = 8, units = "in")

# Compare covariates to DMR methylation ####
samples <- subset(info, WDStage %in% c("Early WD", "Advanced WD"))
samples <- samples[,c("Sequencing.ID", "Sex", "Age", "BMI", "FibrosisStage", "Steatosis", "InflammationGrade", "WDStage")]
colnames(samples)[1] <- "Sample"
samples$Sample <- as.character(samples$Sample)
samples$Sex <- factor(as.character(samples$Sex), levels=c("Male", "Female"), ordered=TRUE)
samples$WDStage <- factor(samples$WDStage, levels=c("Early WD", "Advanced WD"), ordered=TRUE)
...
for(i in 1:dim(meth_heat)[1]){
        ...
        lm <- NULL
        sum <- NULL
        lm <- lm(as.numeric(meth_heat[i,]) ~ samples$FibrosisStage)
        sum <- summary(lm)
        ...
        lm <- NULL
        sum <- NULL
        lm <- lm(as.numeric(meth_heat[i,]) ~ samples$Steatosis)
        sum <- summary(lm)
        ...
        lm <- NULL
        sum <- NULL
        lm <- lm(as.numeric(meth_heat[i,]) ~ samples$InflammationGrade)
        sum <- summary(lm)
        ...
        # WD Advanced vs WD Early
        ttest <- NULL
        ttest <- t.test(meth_heat[i,samples$Sample[samples$WDStage == "Advanced WD"]], 
                        meth_heat[i,samples$Sample[samples$WDStage == "Early WD"]])
...
newCols <- c("chr", "start", "end", paste(rep(c("FvsM"), each=5), c("meanDiff", "confIntL", "confIntR", "tstat", "pvalue"), sep="_"),
             ...
             paste(rep(c("WDStage"), each=5),c("meanDiff", "confIntL", "confIntR", "tstat", "pvalue"), sep="_"))
...
covAssoc$WDStage_pAdj <- p.adjust(covAssoc$WDStage_pvalue, "bonf")
write.table(covAssoc, file="Tables/Liver WDEvWDA DMRs Covariate Associations.txt", sep="\t", quote=FALSE, row.names=FALSE)

# Bonf Significant DMRs
table(covAssoc$FvsM_pAdj < 0.05)              # 0
table(covAssoc$Age_pAdj < 0.05)               # 0
table(covAssoc$BMI_pAdj < 0.05)               # 0
table(covAssoc$FibrosisStage_pAdj < 0.05)     # 1
table(covAssoc$Steatosis_pAdj < 0.05)         # 0
table(covAssoc$InflammationGrade_pAdj < 0.05) # 0
table(covAssoc$WDStage_pAdj < 0.05)           # 10

# Heatmap Sorted by WDEvWDA logp-value ####
pvals <- covAssoc[,c("FvsM_pvalue", "Age_pvalue", "BMI_pvalue", "FibrosisStage_pvalue", "Steatosis_pvalue", "InflammationGrade_pvalue",
                     "WDStage_pvalue")]
...
logPvals_sort <- logPvals[order(logPvals[,"WD Advanced\nvs Early"]),]
plot <- ggheatmap2_sort(x = logPvals_sort, custom.label = newCols, hm.colours = c("Black", "#FF0000"), my.values = c(0,1))
pdf(file="Figures/Liver WDEvWDA DMRs Covariate Association Heatmap sorted.pdf", width=6, height=7, onefile = FALSE)
